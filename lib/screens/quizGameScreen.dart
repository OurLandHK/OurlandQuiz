...
class QuizGameState extends State<QuizGameScreen> {
  ...
  void nextQuestion(BuildContext context) {
    ...
    questionService
        .getQuestion(this._questionIDs[this.questionIndex + 1])
        .then((question) {
      ...
      if (widget.mode == GameModes[FIX_TIME_GAME_INDEX].label ||
        ...
        if (_timer != null) {
          ...
          if (question.totalText() > BaseText) {
            _maxQuestionTime +=
                (((question.totalText() - BaseText) / TextFactor).ceil() *
                    QuestionTimeFactor);
          }
      ...
      TextStyle qtextStyle =
          pickTitleTextStyle(context, question.title, question.imageUrl);
      String maxOption = question.options[0];
      question.options.forEach((element) {
        if (maxOption.length < element.length) maxOption = element;
      });
      setState(() {
  ...
  */
  ...
  initPlatformState() async {
    ...
    //if (!mounted) return;
    questionService
        .getRandomQuestionID(widget.category, widget.totalQuestion)
        .then((ids) {
  ...
  void validateAnswer(BuildContext context) async {
    ...
    if (userAnswerSet
            .toSet()
            .intersection(this._questions[questionIndex].answers.toSet())
            .length !=
        userAnswerSet
            .toSet()
            .union(this._questions[questionIndex].answers.toSet())
            .length) {
      correct = false;
      questionTime = _beginQuestionTime;
    }
    Result _result = new Result(this._questions[questionIndex].id,
        userAnswerSet, questionTime, correct);
    ...
    if ((widget.mode == GameModes[FIX_TIME_GAME_INDEX].label ||
      ...
      if (questionIndex + 1 < _questionIDs.length) {
  ...
  void showResult(BuildContext context, bool correct) async {
    ...
    if (correct) {
      iconWidget =
          Icon(Icons.check_circle, color: Colors.yellow, size: radius * 2);
      answerList = [
    ...
    Widget answerWidget = Container(
      ...
      decoration: BoxDecoration(
        color: MEMO_COLORS[
            this._questions[questionIndex].color + 1 % MEMO_COLORS.length],
  ...
  void startCountdown(BuildContext context, int wait) async {
    ...
    setState(() {
      _overlayWidget =
          Icon(Icons.traffic, color: Colors.yellow, size: radius * 2);
    });
    await Future.delayed(Duration(milliseconds: wait ~/ 5 - 10));
    setState(() {
      _overlayWidget = Container();
    ...
    setState(() {
      _overlayWidget =
          Icon(Icons.traffic, color: Colors.blue, size: radius * 2);
    });
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) => _hideBar(context));
    Widget body = null;
    switch (_gameStage) {
      case 1:
        nextQuestion(context);
    ...
    if (_gameStage < 3) {
      ...
      Widget reportBody = ReportWidget(this._questions, this._examResult);
      if (widget.mode == ValidateRoute) {
        reportBody = SingleChildScrollView(
  ...
  Widget _buildSubmit(BuildContext context) {
    ...
    if (!_submitDisable && _currentQuestionTime > 0) {
      ...
      button = RaisedButton(child: Text(text));
    }
    return button;
  }
  ...
  Widget optionWidget(BuildContext context, int index) {
    ...
    LinearGradient gradient = LinearGradient(colors: [Colors.yellow , Colors.white],
      ...
      tileMode: TileMode.clamp);
    */
    bool correctAnswer = this
        ._questions[this.questionIndex]
        .answers
        .contains(this._options[index]);
