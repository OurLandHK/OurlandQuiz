...
import './historyQuestionScreen.dart';
...
class ViewQuestionScreen extends StatefulWidget {
  ...
  bool readOnly;
  ViewQuestionScreen(
      {@required this.question, @required this.questionId, this.readOnly}) {
    if (readOnly == null) {
      readOnly = false;
    }
    print('screen');
  ...
  State createState() => new ViewQuestionState(
      question: this.question, questionId: this.questionId);
}
...
class ViewQuestionState extends State<ViewQuestionScreen> {
  ...
  getHistoryIds() async {
    if (!widget.readOnly) {
      questionService.getHistoryIds(this.question.id, updateHistoryIds);
    }
  }

  void updateHistoryIds(List<String> ids) {
    setState(() {
      _historyId = ids;
    });
  }
...
//      key: _scaffoldKey,
  ...
  Future webGetImage(WebImagePicker.ImageType outputType) async {
    File newImageFile =
        await WebImagePicker.ImagePickerWeb.getImage(outputType: outputType);
  ...
  Widget tagUI(BuildContext context) {
    ...
    this._tags.forEach((tag) {
      chips.add(Chip(
          label: Text(tag), labelStyle: Theme.of(context).textTheme.subtitle1));
    });
  ...
  Widget optionWidget(BuildContext context, int answerIndex) {
    bool displayResult =
        (this.question != null && user.questionIDs.contains(this.question.id));
  ...
  Widget referenceWidget(BuildContext context, int focusIndex) {
    if (this._reference.length > 0) {
      if (this._reference.contains("http")) {
        ...
        return LinkPreview(
            link: this._reference, launchFromLink: true, hideLink: false);
      } else {
        return TextFormField(
          enabled: _addMode,
  ...
  Widget descDate(BuildContext context, int focusIndex) {
    if (this.question.eventDate != null) {
      return TextFormField(
        enabled: _addMode,
        initialValue: DateFormat('yyyy MMM dd').format(this.question.eventDate),
        focusNode: _focusNodes[focusIndex],
        onFieldSubmitted: (term) {
          fieldFocusChange(
              context, _focusNodes[focusIndex], _focusNodes[focusIndex + 1]);
  ...
  Widget formUI(BuildContext context) {
    ...
    */
    Widget buttonBar = RaisedButton(
        child: Text(textRes.LABEL_EDIT_QUESTION),
        onPressed: () {
          Navigator.of(context).push(
              new MaterialPageRoute<void>(builder: (BuildContext context) {
            return new AddQuestionScreen(question: this.question, edit: true);
          }));
        });
    if (_historyId.length > 0) {
      buttonBar = Row(children: [
        RaisedButton(
            child: Text(textRes.LABEL_EDIT_QUESTION +
                " " +
                _historyId.length.toString()),
            onPressed: () {
              Navigator.of(context).push(
                  new MaterialPageRoute<void>(builder: (BuildContext context) {
                return new HistoryQuestionScreen(
                    question: this.question, historyIds: _historyId);
              }));
            }),
        buttonBar
      ]);
        ...
        child: Column(
          ...
          children: <Widget>[
            Row(children: [
              _questionMemo,
              DateTimeWidget(
                  this.question.created, null, textRes.LABEL_CREATE_TIME),
              tagUI(context)
            ]),
            titleUI(context, 0),
            ...
            descImageUI(context),
            const SizedBox(height: 5.0),
            widget.readOnly? Container() : buttonBar,
            //_buildShare(context)
          ],
        ));
  }
